...
const MessageBox_1 = require("../lib/MessageBox");
...
function main(param) {
  ...
  scene.onLoad.add(() => {
    ...
    MessageBox_1.MessageBox.appendNewFontFace(fontPath, FONTFACE_NAME).then(() => {
      scene.append(getSampleLabel(scene, FONTFACE_NAME));
      const messageBox = new MessageBox_1.MessageBox({
        EParam: {
          scene: scene,
          touchable: true
        },
      ...
      const message = [["メロスは激怒した。必ず、かの邪智暴虐の王を除かなければならぬと決意した。", "メロスには政治がわからぬ。", "メロスは、村の牧人である。"].join("\n"), ["笛を吹き、羊と遊んで暮して来た。けれども邪悪に対しては、人一倍に敏感であった。", "きょう未明メロスは村を出発し、野を越え山越え、十里はなれた此このシラクスの市にやって来た。", "メロスには父も、母も無い。", "女房も無い。"].join("\n"), ["十六の、内気な妹と二人暮しだ。", "この妹は、村の或る律気な一牧人を、近々、花婿として迎える事になっていた。", "結婚式も間近かなのである。"].join("\n")];
...
function getSampleLabel(scene, fontFamily) {
