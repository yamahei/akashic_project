...
function main(param) {
  ...
  scene.onLoad.add(() => {
    const objects = [];
    const names = ["blue_book", "red_book", "green_book", "chest_bright", "chest_red", "chest_gold", "chest_wood", "chest_bronze", "flag_red", "flag_magenda", "flag_blue", "flag_yellow", "door_wood1", "door_wood1_bright", "door_wood2", "door_wood2_bright", "door_wood3", "door_wood3_bright", "door_iron", "door_iron_bright", "switch_p", "switch_ud", "switch_lr", "switch_fb", "lamp", "treasure_pendant", "treasure_key_heart", "treasure_key_silver", "treasure_jewel"];
    names.forEach((name, index) => {
      const setting = ObjectEntity_1.ObjectEntity.getObjectSetting(name);
      const x = index % 5 * 43 + 10;
      const y = Math.floor(index / 5) * 52 + 10;
      const param = {
        scene: g.game.scene(),
        touchable: true,
        x: x,
